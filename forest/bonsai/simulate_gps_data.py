...
def bounding_box(lat: float, lon: float, radius: int) -> tuple[float]:
    """
...
class Person:
    ...
    def __init__(
        ...
        if self.main_employment > 0 and len(self.office_address) > 0:
            self.office_exits = []
            self.office_codes = []
            for possible_exit in ["cafe", "restaurant", "park", "fitness"]:
                for place in getattr(self, possible_exit + "_places"):
                    if (
                        great_circle_dist(
                            self.office_address[0],
                            self.office_address[1],
                            place[0],
                            place[1],
                        )
                        < 500
                        ...
                        self.office_codes.append(possible_exit)
    ...
    def choose_preferred_exit(
        self, t_s: float, update: bool = True
    ) -> tuple[str, tuple]:
        """
    ...
    def choose_preferred_office_exit(
        self, api_key: str
    ) -> tuple[np.ndarray, np.ndarray, str]:
        """
        This function samples through the possible exits
         when the person is at work or university.\n
        Args:
            api_key: str, api key of open route service
        Return:
            np.ndarray, array of nodes to travel
            from office address to location of exit
            np.ndarray, array of nodes to travel back
             from location of exit to office address
            str, exit selected
        """
    ...
    def choose_action(
        self, t_s: float, day_now: int, update: bool = True
    ) -> tuple[str, tuple[float, float], list[int, int], str]:
        """
        ...
        return (
            ...
            location,
            [
                0.5 * 3600 + 1.5 * 3600 * (self.active_status - 1) / 9,
                1 * 3600 + 1.5 * 3600 * (self.active_status - 1) / 9,
            ],
            preferred_exit,
        )
...
def gen_basic_traj(
    ...
    if vehicle == "walk":
        ...
        spd_range = [10, 14]
    distance = great_circle_dist(l_s[0], l_s[1], l_e[0], l_e[1])
    traveled = 0
    t_e = t_s
...
def gen_all_traj(
    ...
    Args:
        house_address: (str) indicating the house address
        of the person to generate
        attributes: (list) contains the attributes
        required to generate a class Person
    ...
    if len(person.possible_exits) < 4 or (
        person.main_employment > 0 and len(person.office_address) == 0
    ):
        return [], [], []
    ...
    while current_date < end_date:
        ...
        if action == "p":
            ...
            go_path, transport = person.calculate_trip(
                person.house_address, location, api_key
            )
            return_path, _ = person.calculate_trip(
                location, person.house_address, api_key
            )
            ...
            # Flight 1
            res1, distance1 = gen_route_traj(go_path, transport2, t_s)
            t_s1 = res1[-1, 0]
            ...
            if (
                ...
                limits2 = [lim * rndm_coefficient for lim in limits]
            ...
            # Flight 2
            res7, distance2 = gen_route_traj(return_path, transport2, t_s6)
            t_s7 = res7[-1, 0]
            traj3 = np.vstack((traj2, res7))

            action3, location3, duration3, exit3 = person.choose_action(
                t_s7, current_weekdate, update=False
            )
            update_exit = False
            if action3 == "fpf":
                if (
                    great_circle_dist(
                        location[0], location[1], location3[0], location3[1]
                    )
                    < 5
                ):
                    update_exit = True
                ...
                elif great_circle_dist(
                    location[0], location[1], location3[0], location3[1]
                ) < great_circle_dist(
                    person.house_address[0],
                    person.house_address[1],
                    location3[0],
                    location3[1],
                ):
                    update_exit = True
            ...
            elif t_s7 - dates_passed_in_hrs < 24 * 3600:
                t_s = t_s7
                traj = np.vstack((traj, traj3))
                total_d += d_temp + distance2

            else:
                # pause
                res = gen_basic_pause(
                    person.house_address, t_s, None, [15 * 60, 30 * 60]
                )
                t_s = res[-1, 0]
                traj = np.vstack((traj, res))
...
def process_attributes(
    attributes: dict[int, list], key: str, user: int
) -> tuple[list, dict]:
    """
    ...
    if "vehicle" in attributes[key].keys():
        if attributes[key]["vehicle"] in vehicle_dict:
            ...
            print(
                "For User "
                + str(user)
                + " vehicle was not in ['foot', 'car', 'bicycle']"
            )
            return []
    ...
    if "main_employment" in attributes[key].keys():
        if attributes[key]["main_employment"] in main_employment_dict:
            ...
            print(
                "For User "
                + str(user)
                + " main_employment was not in ['none', 'work', 'student']"
            )
            return []
        ...
        for possible_exit in attributes[key]["preferred_exits"]:
            if possible_exit not in possible_exits_list:
                print(
                    ...
                    + " exit "
                    + possible_exit
                    + " is not in ['cafe', 'bar', 'restaurant',"
                    + " 'park', 'cinema', 'dance', 'fitness']"
                )
...
def sim_gps_data(
    ...
    try:
        ...
        print_msg = "Overpass query came back empty. "
        print_msg += "Check that the location argument"
        print_msg += ", ISO code and city name, did not have any misspellings."
        print(print_msg)
        sys.exit()
